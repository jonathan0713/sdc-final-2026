...
def run_grid_search(
    ...
    rows = []
    combinations = list(product(tracker_modes, max_distances, max_ages, min_hits_values))
    ...
    for combo_idx, (tracker_mode, max_distance, max_age, min_hits) in enumerate(combinations, start=1):
        combo_name = f"{tracker_mode}_md_{max_distance:g}_age_{max_age}_hits_{min_hits}".replace(".", "p")
        combo_root = grid_root / combo_name

        print("------------------------------------------------------------")
        print(
            f"[{combo_idx}/{len(combinations)}] "
            f"tracker_mode={tracker_mode}, max_distance={max_distance}, "
